...
function normalizeTier(data) {
  ...
  for (const tier of ['tutorial', 'easy', 'medium', 'hard']) {
    out[tier] = (data[tier] || []).map((p) => normalizePuzzleInput(p, false))
  }
  return out
}
...
function loadCompletions(dateKey) {
  return [0, 1, 2].map((i) => ['1', '2'].includes(localStorage.getItem(`scuttlebug:${dateKey}:${i}`)))
}

function loadPerfects(dateKey) {
  return [0, 1, 2].map((i) => localStorage.getItem(`scuttlebug:${dateKey}:${i}`) === '2')
}

function getStoredPushCount(dateKey, idx) {
  const v = localStorage.getItem(`scuttlebug:${dateKey}:${idx}:moves`)
  return v != null ? parseInt(v, 10) : null
}

function savePushCount(dateKey, idx, pushes) {
  localStorage.setItem(`scuttlebug:${dateKey}:${idx}:moves`, String(Math.min(pushes, MAX_PUSH_DISPLAY)))
}
...
function markComplete(dateKey, idx, pushesThisRun, puzzleMinPushes) {
  ...
  const key = `scuttlebug:${dateKey}:${idx}`
...
function loadMoveCounts(dateKey) {
  return [0, 1, 2].map((i) => {
    const v = localStorage.getItem(`scuttlebug:${dateKey}:${i}:moves`)
    return v != null ? parseInt(v, 10) : null
  })
}
...
function storageKeyGameState(dateKey, puzzleIndex) {
  return `scuttlebug:${dateKey}:${puzzleIndex}:gameState`
}
...
export default function Scuttlebug() {
  const chrome = getGameChrome(GAME_KEYS.SCUTTLEBUG)
  ...
  const [mode, setMode] = useState(
    () => getInitialTutorialNav(GAME_KEYS.SCUTTLEBUG, puzzleData.tutorial ?? []).mode
  )
  const [tutorialIdx, setTutorialIdx] = useState(
    () => getInitialTutorialNav(GAME_KEYS.SCUTTLEBUG, puzzleData.tutorial ?? []).tutorialIdx
  )
  ...
  const tierSlots = useMemo(() => {
    ...
    return getEnabledTierIndices(GAME_KEYS.SCUTTLEBUG)
  }, [suitePrefsEpoch])
  ...
  const canShareHub = useMemo(() => {
    void completions
    return hasShareableHubProgress(GAME_KEYS.SCUTTLEBUG, daily.key)
  ...
  const { hasSeenInstructions, showInstructions, setShowInstructions, closeInstructions } =
    useInstructionsGate('scuttlebug:hasSeenInstructions', {
      openOnMount: !curateMode,
      completionStoragePrefix: 'scuttlebug',
  ...
  const resetFromData = useCallback((data, restore) => {
    if (!data) return
    if (restore?.pieces && restore?.player) {
      setPlay({
  ...
  useEffect(() => {
    if (!curateMode) persistTutorialResumeState(GAME_KEYS.SCUTTLEBUG, mode, tutorialIdx)
  }, [curateMode, mode, tutorialIdx])

  useEffect(() => {
    if (curateMode || mode !== 'daily') return
    persistHubDailySlot(GAME_KEYS.SCUTTLEBUG, daily.key, dailyIdx)
  }, [curateMode, mode, daily.key, dailyIdx])

  useEffect(() => {
    if (curateMode || mode !== 'daily') return
    const c = clampDailyIndexToTierPrefs(GAME_KEYS.SCUTTLEBUG, dailyIdx)
  ...
  useEffect(() => {
    ...
    if (
      ...
      return
    if (
      play.player.r !== currentPuzzleData.target.r ||
      play.player.c !== currentPuzzleData.target.c
    )
      return
  ...
  useEffect(() => {
    if (curateMode || mode !== 'daily') return
    const done = isSuiteCompleteForPrefs(GAME_KEYS.SCUTTLEBUG, daily.key)
    if (allDailyDoneCompletionRef.current === null) {
      allDailyDoneCompletionRef.current = done
      return
    }
    if (done && !allDailyDoneCompletionRef.current) {
      window.setTimeout(() => setShowCompletionModal(true), SCUTTLEBUG_SUITE_MODAL_MS)
    }
    allDailyDoneCompletionRef.current = done
  }, [curateMode, mode, completions, daily.key, suitePrefsEpoch])

  const suiteDone = isSuiteCompleteForPrefs(GAME_KEYS.SCUTTLEBUG, daily.key)
  ...
  useSuiteCompletionTimer(GAME_KEYS.SCUTTLEBUG, daily.key, {
    track: !curateMode && mode === 'daily',
    alreadyFullyComplete: isSuiteCompleteForPrefs(GAME_KEYS.SCUTTLEBUG, daily.key),
  ...
  const handlePrimary = () => {
    ...
    if (mode === 'tutorial') {
      if (tutorialIdx < puzzleData.tutorial.length - 1) setTutorialIdx((i) => i + 1)
        ...
        setMode('daily')
        setDailyIdx(clampDailyIndexToTierPrefs(GAME_KEYS.SCUTTLEBUG, 0))
      }
    } else {
      const next = nextIncompleteEnabledTierExcluding(GAME_KEYS.SCUTTLEBUG, daily.key, dailyIdx)
      if (next !== null) {
        setSolved(false)
        setCelebrating(false)
        setDailyIdx(next)
  ...
  const handleStatsClick = useCallback(() => {
    if (curateMode) {
      ...
      if (!entry || !p) return
      const text = formatCurateClipboard('scuttlebug', entry.tier, entry.indexInTier + 1, p, 200)
  ...
  return (
    <div className="game-container scuttlebug-game">
      <style>{`
        .scuttlebug-game {
          --sb-ink: #111;
          --sb-green: #16a34a;
        ...
        .scuttlebug-game #scuttlebug-canvas-wrap {
          width: 100%;
          aspect-ratio: 1 / 1;
          position: relative;
          margin: 0 auto;
        ...
        .scuttlebug-game .grid-overlay,
        .scuttlebug-game .grid-line {
          pointer-events: none;
        }
        .scuttlebug-game .grid-overlay {
          position: absolute;
        ...
        .scuttlebug-game .grid-line.target::after {
          content: "";
          position: absolute;
          inset: 24%;
          border-radius: 50%;
        ...
        .scuttlebug-game .piece-block::after {
          content: "";
          position: absolute;
          inset: 6px;
        ...
        .scuttlebug-game .player-layer svg {
          width: 100%;
          height: 100%;
          overflow: visible;
        }
        @keyframes scuttlebug-celebrate {
        ...
        .scuttlebug-game .player-layer.celebrating {
          animation: scuttlebug-celebrate 700ms ease-in-out 1;
        }
        .scuttlebug-game .stats-num.at-par { color: var(--sb-green); }
      `}</style>
      ...
      <div className="game-stage">
        <div
          id="scuttlebug-canvas-wrap"
          ref={wrapperRef}
          style={{
            maxWidth: `min(460px, calc(100dvh - 300px), ${gridSize * SCUTTLEBUG_MAX_CELL_PX}px)`,
          }}
