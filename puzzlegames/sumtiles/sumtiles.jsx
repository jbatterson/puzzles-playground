...
export default function SumTiles() {
  const chrome = getGameChrome(GAME_KEYS.SUMTILES)
  ...
  const [mode, setMode] = useState(
    () => getInitialTutorialNav(GAME_KEYS.SUMTILES, puzzleData.tutorial ?? []).mode
  )
  const [tutorialIdx, setTutorialIdx] = useState(
    () => getInitialTutorialNav(GAME_KEYS.SUMTILES, puzzleData.tutorial ?? []).tutorialIdx
  )
  ...
  const canShareHub = useMemo(
    () => hasShareableHubProgress(GAME_KEYS.SUMTILES, daily.key),
  ...
  const { hasSeenInstructions, showInstructions, setShowInstructions, closeInstructions } =
    useInstructionsGate('sumtiles:hasSeenInstructions', {
      openOnMount: !curateMode,
      completionStoragePrefix: 'sumtiles',
    ...
    if (done && !allDailyDoneCompletionRef.current) {
      ...
      if (!s?.solveAnim) {
        ...
        window.setTimeout(
          ...
          SUMTILES_SUITE_MODAL_SOLVE_ANIM_MAX_MS
        )
      }
    }
  ...
  const handleStatsClick = useCallback(() => {
    if (curateMode) {
      ...
      if (!entry || !p) return
      const text = formatCurateClipboard('sumtiles', entry.tier, entry.indexInTier + 1, p, 200)
