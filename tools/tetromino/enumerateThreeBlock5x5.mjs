/**
 * Enumerate 5×5 three-tetromino Dung Beetle puzzles with pushes+ball ≥ 7 and pushes ≥ 3.
 *
 ...
 * minMoved = 2 (not all three pieces need to move).
 *
 *   node tools/tetromino/enumerateThreeBlock5x5.mjs
 *   node tools/tetromino/enumerateThreeBlock5x5.mjs --wanted-per-cell=50 --attempts=500000
 */
...
const progressAbs = path.join(repoRoot, 'tools/reports/enumerate-5x5-3block-progress.json')
...
log(
  `Enumerate 5×5 three-block: ${TRIPLES.length} triples × ${BANDS.length} bands = ${cells.length} cells` +
    `\n  wanted/cell=${WANTED_PER_CELL} attempts=${ATTEMPT_CAP} batch=${BATCH_SIZE} minMoved=2 seed=${SEED}`
)
...
const easy5 = data.easy.filter((p) => p.size === 5 && pieceCount(p) === 3)
const inBand = easy5.filter((p) => (p.pushes ?? 0) >= 3 && (p.pushes ?? 0) + (p.ballPushes ?? 0) >= 7)
log(`Easy 5×5 three-block: ${easy5.length} (meeting ≥3 block & total≥7: ${inBand.length})`)
