/**
 * Enumerate 6×6 three-tetromino Dung Beetle puzzles for the hard tier.
 *
 * Keep after canonicalize:
 *   pushes ≥ 6 (= 2N), blocksMoved ≥ 3, pushes+ball ∈ [12, 24], glyphs ∈ [30, 70]
 *
 * Search grid: every unordered distinct triple × bands
 *   (block≥12,ball≥0), (10,2), (8,4), (6,6)
 * Per cell: keep up to 50, or stop after 500k attempts (barren if 0).
 * Every ~50 new keepers: canonicalize each, append to hard (after preserved 5×5),
 *   then end-state D4 cull keeping max pushes → max ballPushes → min glyphs.
 * minMoved = 3.
 *
 * On a fresh run (no progress file / --reset): hard is reduced to size-5 rows only.
 *
 *   node tools/tetromino/enumerateThreeBlock6x6Hard.mjs
 *   node tools/tetromino/enumerateThreeBlock6x6Hard.mjs --wanted-per-cell=50 --attempts=500000
 */
...
const progressAbs = path.join(repoRoot, 'tools/reports/enumerate-6x6-3block-hard-progress.json')
...
const MAX_MOVES = num('max-moves', 90)
...
const MIN_GLYPHS = 30
const MAX_GLYPHS = 70
/** Soft prefilter before canonicalize (official length often shrinks). */
...
function hardPartition(hard) {
  const five = []
  const sixPlus = []
  for (const raw of hard || []) {
    if (raw.size === 5) five.push(raw)
    else sixPlus.push(raw)
  }
  return { five, sixPlus }
}
...
async function flushBatch(pending, data) {
  ...
  const { five, sixPlus } = hardPartition(data.hard)
  const merged = {
    tutorial: data.tutorial,
    easy: data.easy,
    medium: data.medium,
    hard: [...five, ...sixPlus, ...append],
  }
  const { next, clusters, dropped, failed } = cullEndStateDupes(merged)
  // Re-assert 5×5 hard stay first after cull.
  const parts = hardPartition(next.hard)
  next.hard = [...parts.five, ...parts.sixPlus]
  writePuzzles(next)
  log(
    `  flush: appended ${append.length}, canonFail=${canonFail}, end-dupe clusters=${clusters} dropped=${dropped}` +
      (failed ? ` replayFail=${failed}` : '') +
      ` → hard ${next.hard.length} (5×5=${parts.five.length}, 6×6+=${parts.sixPlus.length})`
  )
...
const hadProgress = fs.existsSync(progressAbs) && !RESET
...
if (!hadProgress || RESET) {
  const before = data.hard.length
  const five = data.hard.filter((p) => p.size === 5)
  data = { ...data, hard: five }
  writePuzzles(data)
  if (RESET && fs.existsSync(progressAbs)) fs.unlinkSync(progressAbs)
  progress = { seed: SEED, doneCells: [], cellResults: {} }
  log(`Hard reset to 5×5 only: ${before} → ${five.length}`)
}
...
log(
  `Enumerate 6×6 three-block hard: ${TRIPLES.length} triples × ${BANDS.length} bands = ${cells.length} cells` +
    `\n  wanted/cell=${WANTED_PER_CELL} attempts=${ATTEMPT_CAP} batch=${BATCH_SIZE}` +
    ` minMoved=${MIN_MOVED} total=${MIN_TOTAL}–${MAX_TOTAL} glyphs=${MIN_GLYPHS}–${MAX_GLYPHS} seed=${progress.seed || SEED}`
)
log(
...
for (const cell of cells) {
  ...
  const ballPulls = 2 * Math.max(cell.minPushes, 6)
  ...
  while (kept < WANTED_PER_CELL && attempts < ATTEMPT_CAP) {
    ...
    if (g < SOFT_MIN_GLYPHS || g > SOFT_MAX_GLYPHS || g > MAX_MOVES) {
      glyphBand++
      continue
    }
    ...
    log(
      `  kept ${kept}/${WANTED_PER_CELL}: ${result.pushes}b+${result.ballPushes}ball ` +
        `glyphs=${g} (attempts ${attempts}, pending ${pending.length})`
    )
...
const parts = hardPartition(finalCull.next.hard)
finalCull.next.hard = [...parts.five, ...parts.sixPlus]
...
log(
  `Hard split: 5×5=${parts.five.length}, 6×6+=${parts.sixPlus.length}` +
    ` | end-dupe clusters=${finalCull.clusters} dropped=${finalCull.dropped}`
)

const hard6 = data.hard.filter((p) => p.size === 6 && pieceCount(p) === 3)
const inBand = hard6.filter(meetsHardCriteria)
log(`Hard 6×6 three-block: ${hard6.length} (meeting criteria: ${inBand.length})`)
